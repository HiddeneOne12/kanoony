...
class BlogListingBody extends ConsumerStatefulWidget {
  const BlogListingBody({super.key});

  @override
  ConsumerState<BlogListingBody> createState() => _BlogListingBodyState();
}

class _BlogListingBodyState extends ConsumerState<BlogListingBody> {
  @override
  void initState() {
                        ...
                        dashboardVariables.isLoaded
                            ...
                            : ListView.builder(
                                itemCount: dashboardVariables
                                    .blogs.length, // Length of the data list
                                shrinkWrap: true,
                                padding: EdgeInsets.only(top: 15.h),
                                physics: const NeverScrollableScrollPhysics(),
                                itemBuilder: (context, index) {
                                  var data = dashboardVariables.blogs[index];
                                  return Blog(
                                    date:
                                        dashboardVariables.staticData?.blogs ??
                                            '',
                                    title: data.title,
